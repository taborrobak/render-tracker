class RenderFlow {
        ...
        initEventListeners() {
          ...
          // Base directory input listener
          const baseDirInput = document.getElementById("baseDirectory");
          if (baseDirInput) {
            baseDirInput.addEventListener("input", () => {
              // Refresh preview if a job is currently selected
              if (this.selectedJobId) {
                this.showJobDetails(this.jobs.find(job => job.id === this.selectedJobId));
              }
            });
          }
        }
        ...
        async renderJobDetail() {
          ...
          try {
            const baseDir = document.getElementById('baseDirectory').value || 'file:///home/tabor/render_tracker/renders/';
            const response = await fetch(`/preview/${job.id}?base_dir=${encodeURIComponent(baseDir)}`);
            ...
            const html = `
                        ...
                        <div class="job-detail-content">
                            <div class="preview-section">
                                <!-- Preview image hidden for file:// URL approach -->
            ...
            // Hide preview image loading - just show file link
        ...
        updateFlaggedPopup() {
          ...
          if (flaggedJobs.length === 0) {
            ...
            if (resetButton) {
              ...
              resetButton.textContent = "🔄 No Flagged Jobs to Reset";
            }
          } else {
        ...
        async resetAllFlaggedJobs() {
          ...
          try {
            ...
            if (response.ok) {
              ...
              this.updateFlaggedPopup();
              console.log("🔄 Flagged popup updated");
              
              // Show success message (optional)
              if (result.reset_count > 0) {
                // Could add a toast notification here if desired
                console.log(`✅ Reset ${result.reset_count} flagged jobs to inactive status`);
              }
            } else {
