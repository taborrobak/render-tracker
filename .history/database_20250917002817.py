...
class JobDatabase:
    ...
    def get_jobs(self, limit: int = 20, offset: int = 0, status: Optional[str] = None) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            if status:
                jobs = conn.execute(
                    "SELECT id, status, created_at, updated_at, start_time FROM jobs WHERE status = ? ORDER BY id LIMIT ? OFFSET ?",
                    (status, limit, offset)
                ).fetchall()
            else:
                jobs = conn.execute(
                    "SELECT id, status, created_at, updated_at, start_time FROM jobs ORDER BY id LIMIT ? OFFSET ?",
                    (limit, offset)
                ).fetchall()
    ...
    def get_job_by_id(self, job_id: int) -> Optional[Dict]:
        ...
        with self.get_connection() as conn:
            job = conn.execute(
                "SELECT id, status, created_at, updated_at FROM jobs WHERE id = ?",
                (job_id,)
            ).fetchone()
    ...
    def claim_job(self, job_id: int) -> bool:
        ...
        with self.get_connection() as conn:
            cursor = conn.execute(
                "UPDATE jobs SET status = 'working', updated_at = ? WHERE id = ? AND status = 'inactive'",
