...
# Configuration for base URL (for deployment)
BASE_URL = os.getenv("BASE_URL", "http://localhost:8000")

# For Railway deployment, use RAILWAY_PUBLIC_DOMAIN if available
...
async def get_job_preview(job_id: int, request: Request):
    # Get the base URL from request or configuration
    base_url = BASE_URL
    if request:
        # Use the actual request URL for dynamic base URL
    ...
    return {
        "image_url": f"{base_url}/renders/{job_id}.png",
        "traits": traits_data
    }
