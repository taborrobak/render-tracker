...
class JobDatabase:
    ...
    def get_jobs(self, limit: int = 20, offset: int = 0, status: Optional[str] = None) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            if status:
                jobs = conn.execute(
                    "SELECT id, status, created_at, updated_at FROM jobs WHERE status = ? ORDER BY id LIMIT ? OFFSET ?",
                    (status, limit, offset)
                ).fetchall()
            else:
                jobs = conn.execute(
                    "SELECT id, status, created_at, updated_at FROM jobs ORDER BY id LIMIT ? OFFSET ?",
                    (limit, offset)
                ).fetchall()
